...
//! one combined FTL file per locale directory — one identical copy per
//! consumer crate, because `i18n_embed_fl::fl!` resolves the file as
//! `<locale>/<CARGO_PKG_NAME>.ftl` in the crate that invokes it. It also
...
/// Every crate that calls `t!`/`fl!` against these locales. Add new bar
/// module crates here.
const DOMAINS: &[&str] = &[
    "wayle-shell-core",
    "wayle-shell",
...
fn concatenate_partials(locale_dir: &Path) {
    ...
    for domain in DOMAINS {
        let output = locale_dir.join(format!("{domain}.ftl"));
