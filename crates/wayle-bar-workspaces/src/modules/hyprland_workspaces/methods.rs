...
impl HyprlandWorkspaces {
    ...
    pub fn dispatch_click_action(&self, action: WorkspaceClickAction, id: WorkspaceId) {
        match action {
    ...
    pub fn spawn_load_workspace_rules(
    ...
    pub fn sync_after_active_workspace_change(&mut self, has_min_workspace_count: bool) {
    ...
    pub fn update_app_icons_on_title_change(&mut self) {
        let config = self.config.config();
        let ws_config = &config.modules.hyprland_workspaces;
    ...
    pub fn switch_to_workspace(&self, id: WorkspaceId) {
        let Some(hyprland) = &self.hyprland else {
            return;
        };
    ...
    pub fn navigate_workspace(&self, direction: i64) {
        let Some(hyprland) = &self.hyprland else {
            return;
        };
    ...
    pub fn stop_blink_if_no_urgent(&mut self) {
    ...
    pub fn prune_stale_urgent_windows(&mut self) {
        if self.urgent_windows.is_empty() {
            return;
        }
