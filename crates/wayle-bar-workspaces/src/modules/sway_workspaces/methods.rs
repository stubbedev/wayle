//! [`SwayWorkspaces`] private impl methods: rebuild, border classes, action dispatch.
...
use wayle_sway::{WorkspaceRef, core::Window};

use super::{
    BLINK_INTERVAL, SwayWorkspaces, SwayWorkspacesCmd,
    button::{AppIconInit, SwayWorkspaceButtonInit},
...
impl SwayWorkspaces {
    ...
    pub fn rebuild_buttons(&mut self) {
        let workspaces = self.sway.workspaces.get();
        let windows = self.sway.windows.get();
        let config = self.config.config();
        let ws_config = &config.modules.sway_workspaces;

        let occupied = occupied_workspace_ids(&windows);
        ...
        let snapshots = workspaces
            ...
            .map(|workspace| {
                ...
                WorkspaceSnapshot {
                    id,
                    num: workspace.num.get(),
                    name: Some(workspace.name.get()),
                    output: Some(workspace.output.get()),
                    is_urgent: workspace.is_urgent.get(),
                    is_active: workspace.is_active.get(),
                    is_focused: workspace.is_focused.get(),
                    has_windows: occupied.contains(&id),
                }
            })
            .collect();
        ...
        sender.command(move |out, shutdown| async move {
            ...
            loop {
                tokio::select! {
                    ...
                    _ = interval.tick() => {
                        let _ = out.send(SwayWorkspacesCmd::BlinkTick);
                    }
                }
            }
        });
    ...
    pub fn update_border_classes(&self, show_border: bool) {
    ...
    pub fn dispatch_click_action(&self, action: WorkspaceClickAction, click_id: u64) {
        match action {
            WorkspaceClickAction::None => {}
            WorkspaceClickAction::FocusWorkspace => self.spawn_focus(WorkspaceRef::Id(click_id)),
            WorkspaceClickAction::FocusNext => self.spawn_focus_next(),
            WorkspaceClickAction::FocusPrevious => self.spawn_focus_previous(),
            WorkspaceClickAction::FocusLast => self.spawn_focus_last(),
    ...
    pub fn dispatch_scroll_action(&self, action: WorkspaceClickAction) {
        match action {
            WorkspaceClickAction::None => {}
    ...
    fn spawn_focus(&self, reference: WorkspaceRef) {
        let sway = self.sway.clone();
        tokio::spawn(async move {
            if let Err(err) = sway.focus_workspace(reference.clone()).await {
                warn!(error = %err, ?reference, "sway focus_workspace failed");
            }
        });
    }

    fn spawn_focus_next(&self) {
        let sway = self.sway.clone();
        tokio::spawn(async move {
            if let Err(err) = sway.focus_next_on_output().await {
                warn!(error = %err, "sway focus next_on_output failed");
            }
        });
    }

    fn spawn_focus_previous(&self) {
        let sway = self.sway.clone();
        tokio::spawn(async move {
            if let Err(err) = sway.focus_prev_on_output().await {
                warn!(error = %err, "sway focus prev_on_output failed");
            }
        });
    }

    fn spawn_focus_last(&self) {
        let sway = self.sway.clone();
        tokio::spawn(async move {
            if let Err(err) = sway.focus_back_and_forth().await {
                warn!(error = %err, "sway focus back_and_forth failed");
            }
        });
    }
}
...
fn occupied_workspace_ids(windows: &HashMap<u64, Arc<Window>>) -> HashSet<u64> {
    windows
        .values()
        .filter_map(|window| window.workspace_id.get())
        .collect()
}
...
fn windows_on_workspace(
    ...
    result.sort_by_key(|window| window.id.get());
    result
}
