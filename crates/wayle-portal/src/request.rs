...
//! method. The backend exports a Request object there so the app can cancel an
//! in-flight interaction via `Close()`. We mount it for the duration of a
//! method call and remove it on drop ([`RequestGuard`]).
...
/// Mounts a [`Request`] at `handle` and removes it when dropped, so the object
/// never outlives the method call it guards.
...
impl Drop for RequestGuard {
    fn drop(&mut self) {
        ...
        // Removal is async; detach it so Drop stays sync.
