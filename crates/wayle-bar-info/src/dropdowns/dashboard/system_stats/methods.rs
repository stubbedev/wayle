...
pub fn update_usage_ring(ring: &Controller<ProgressRing>, usage: f32, warning: f32, error: f32) {
    let fraction = f64::from(usage / PERCENT_DIVISOR);
...
pub fn update_temp_ring(ring: &Controller<ProgressRing>, celsius: f32, warning: f32, error: f32) {
    let fraction = f64::from((celsius / MAX_TEMP_DISPLAY).clamp(0.0, 1.0));
