//! Live CSS provider applied to each `SwayWorkspaces` component instance.
...
use wayle_config::{
    ConfigService,
    schemas::{
        modules::sway_workspaces::{ICON_BASE_REM, LABEL_BASE_REM},
...
pub(super) fn apply_styling(
    ...
    for (key, style) in &ws_config.workspace_map.get() {
        ...
        for selector_class in classes_for_key(key) {
            css.push_str(&format!(
                ".workspaces.sway .workspace.{selector_class} {{ --ws-override-color: {color_css}; }}"
            ));
        }
    }
...
///
/// Numeric keys generate both `ws-id-N` (stable id) and `ws-name-N` (numeric
/// workspace name) so users can key by either without picking the right one.
/// Non-numeric keys only generate `ws-name-K`.
