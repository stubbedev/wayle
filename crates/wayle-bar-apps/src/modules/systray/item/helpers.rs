...
pub fn select_best_pixmap(pixmaps: &[IconPixmap]) -> Option<&IconPixmap> {
    pixmaps
        .iter()
        .min_by_key(|p| (p.width - TARGET_ICON_SIZE).abs() + (p.height - TARGET_ICON_SIZE).abs())
}

pub fn create_texture_from_pixmap(pixmap: &IconPixmap) -> Option<gdk::Texture> {
...
pub fn hash_pixmaps(pixmaps: &[IconPixmap]) -> u64 {
...
pub fn load_scaled_texture_from_file(path: &str) -> Option<gdk::Texture> {
    let pixbuf =
...
pub fn find_icon_in_theme_path(theme_path: &str, icon_name: &str) -> Option<String> {
...
pub fn load_icon_from_theme_path(theme_path: &str, icon_name: &str) -> Option<gdk::Texture> {
